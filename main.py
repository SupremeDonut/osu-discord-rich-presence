...
class Presence:
	...
	def _connect(self):
		...
		for i in range(10):
			...
			raise RuntimeError("Failed to connect to Discord pipe (is Discord open?)")
		self.path = path

	def _handshake(self):
		self.send({'v': 1, 'client_id': self.client_id}, self.HANDSHAKE)
		ret_op, ret_data = self.recv()
		if ret_op == self.FRAME and ret_data["cmd"] == "DISPATCH" and ret_data["evt"] == "READY":
	...
	def send(self, data, op):
		try:
			...
			self._f.flush()
		except OSError: # happens sometimes
			import sys
			sys.exit("Connection to Discord interrupted")
...
class Client:
	...
	def get_player(self, player):
		resp = requests.get("https://osu.ppy.sh/api/v2/users/" + player, headers=self.headers)
		resp.raise_for_status()
		return resp.json()
...
class Activity:
	...
	def get_presence(cls, data):
		...
		if bm["set"] > 0:
			...
			act["assets"].update({"large_image": url, "large_text": cls.song_name(song, True, False)})
		else:
			act["assets"].update({"large_image": "https://github.com/ppy/osu/blob/master/assets/lazer-nuget.png?raw=true", "large_text": cls.song_name(song, True, False)})

		match state:
			case State.MAINMENU | State.GAMESHUTDOWN | State.GAMESTARTUP | State.PROCESSINGBEATMAPS | State.MULTIROOMS:
				act.update({"details": "In menus", "state": cls.song_name(song, diff=False)})

			case State.SONGSELECTEDIT:
				act.update({"details": "Selecting a beatmap to edit", "state": cls.song_name(song)})
				act["buttons"].append({"label": cls.song_stats(stats), "url": cls.beatmap_link(data)})
			
			case State.EDITINGMAP: # editing
				act.update({"details": "Editing a beatmap", "state": cls.song_name(song, diff=False)})

			case State.SONGSELECT | State.MULTISONGSELECT: # song select
				act.update({"details": f"Selecting a beatmap (+{data['menu']['mods']['str']})", "state": cls.song_name(song)})
				act["buttons"].append({"label": cls.song_stats(stats), "url": cls.beatmap_link(data)})
			...
			case State.PLAYING: # playing
				if play["name"] == user_stats["username"]:
					act.update({"details": f"Playing a beatmap (+{data['menu']['mods']['str']})", "state": cls.song_name(song)})
				else:
					act.update({"details": f"Watching a replay by {play['name']} (+{data['menu']['mods']['str']})", "state": cls.song_name(song)})
				act["buttons"].append({"label": f"{play['hits']['grade']['current']} {play['accuracy']:.2f}% {play['combo']['current']}x {play['pp']['current']}pp {int(play['hits']['unstableRate'])}UR", "url": cls.beatmap_link(data)})
			
			case State.RESULTSSCREEN | State.MULTIRESULTSSCREEN | State.RANKINGTAPCOOP | State.RANKINGTEAM:
				if play["name"] == user_stats["username"]:
					act.update({"details": f"Finished beatmap (+{data['menu']['mods']['str']})", "state": cls.song_name(song)})
				else:
					act.update({"details": f"Finished watching a replay by {play['name']} (+{data['menu']['mods']['str']})", "state": cls.song_name(song)})
				act["buttons"].append({"label": f"{play['hits']['grade']['current']} {play['accuracy']:.2f}% {play['combo']['max']}x {play['pp']['current']}pp {int(play['hits']['unstableRate'])}UR", "url": cls.beatmap_link(data)})
			
			case State.DIRECT: # osu!direct
				act.update({"details": "Browsing osu!direct", "state": cls.song_name(song)})
...
def main():
	...
	subprocess.Popen("gosu\\gosumemory")
	socket = websocket.WebSocketApp("ws://localhost:24050/ws", 
